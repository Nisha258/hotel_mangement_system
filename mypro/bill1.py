...
class bill:
    def __init__(self, win):
        ...
        bill_no_tk = StringVar()
        bill_no_tk.set(str(bill_no))
        calc_var = StringVar()

        customer_nm = StringVar()
        customer_contact = StringVar()
        date_pr = StringVar()
        room_type = StringVar()
        room_qunty = StringVar()
        cost_c = StringVar()
        ...
        self.cust_nm_ent = Entry(self.entry_frame,bd=5,textvariable=customer_nm,font=('Arial',15))
        ...
        self.cust_con_ent = Entry(self.entry_frame,bd=5,textvariable=customer_contact,font=('Arial',15))
        ...
        rows = mycur.fetchall()
        combo = ttk.Combobox(self.entry_frame, textvariable=room_type, font=("times new roman", 16), width=19)
        combo["value"] = rows
        combo.current(0)
        combo.grid(row=4, column=1, padx=2, pady=2)


        self.roomqty_lbl = Label(self.entry_frame,text="Rooms ",font=('Arial',15),bg="lightgrey")
        self.roomqty_lbl.grid(row=5,column=0,padx=2,pady=2)
        self.roomqty = Entry(self.entry_frame, bd=5, textvariable=room_qunty, font=('Arial', 15))
        self.roomqty.grid(row=5, column=1, padx=2, pady=2)
        ...
        self.cost_ent = Entry(self.entry_frame,bd=5,textvariable=cost_c,font=('Arial',15))
        ...
        def getbill():
            ...
            self.bill_txt.insert(END,f"\nCustomer Name : {customer_nm.get()}")
            self.bill_txt.insert(END, f"\nCustomer Contact : {customer_contact.get()}")
            self.bill_txt.insert(END, f"\nDate : {date_pr.get()}")
            self.bill_txt.insert(END,"\n==================================================================================")
        ...
        def add_fun():
            if room_type.get() == "" or room_qunty.get() == "":
                ...
                try:
                    ...
                    db = mysql.connector.connect(host="localhost", user="root", passwd="", database="hoteldb")
                    mycur = db.cursor()
                    mycur.execute("insert into bill1 values(%s,%s,%s,%s,%s,%s,%s)",
                                  ( str(bill_no_tk.get()),str(customer_nm.get()), str(customer_contact.get()), str(date_pr.get()),
                                    str(room_type.get()), str(room_qunty.get()), str(cost_c.get())
                                    ))
                ...
                # fetch data
                qty = int(room_qunty.get())
                cost = int(cost_c.get())
                total = qty * cost
                total_list.append(total)
                self.bill_txt.insert(END,f"\n{room_type.get()}\t\t\t{room_qunty.get()}\t\t\tRs.{cost_c.get()}\t\t\tRs.{total}")
        ...
        def save_func():
            admin = messagebox.askyesno("Confirm?",f"Do you want to save the bill {bill_no_tk.get()}")
            if admin > 0:
                self.bill_content = self.bill_txt.get("1.0",END)
                con = open(f"{sys.path[0]}/E:/12feb/mypro/bills/"+str(bill_no_tk.get())+".txt","w")
                con.write(self.bill_content)
                con.close()
                messagebox.showinfo("successfully",f"Bill {bill_no_tk.get()} has been saved successfully",parent=self.win)
            else:
                return
        ...
        self.btnclr.bind("<Button-1>", press_btn)
